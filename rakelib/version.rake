...
def poetry_version_file = "lib/poetry/charts/version.rb"
...
namespace :version do
  desc "Fail if package.json or its lock disagree with Poetry::Charts::VERSION"
  task :verify do
    ...
    if drift.empty?
      ...
      abort "version drift from Poetry::Charts::VERSION #{gem_version}: #{listed} - " \
            "run `bundle exec rake \"version:bump[#{gem_version}]\"` and commit"
    end
  end

  desc "Fail unless the pushed tag (GITHUB_REF_NAME) is v<Poetry::Charts::VERSION> (the release guard)"
  task :verify_tag do
    tag = ENV.fetch("GITHUB_REF_NAME") { abort "version:verify_tag reads GITHUB_REF_NAME (the pushed tag)" }
    expected = "v#{poetry_gem_version}"
    abort "tag #{tag} does not match Poetry::Charts::VERSION (#{expected})" unless tag == expected

    puts "tag #{tag} matches Poetry::Charts::VERSION"
  end

  desc "Set Poetry::Charts::VERSION and the npm package version together: rake \"version:bump[X.Y.Z]\""
  task :bump, [:version] do |_, args|
    ...
    puts "bumped Poetry::Charts::VERSION and package.json to #{version}; commit, then tag v#{version}"
  end
end
